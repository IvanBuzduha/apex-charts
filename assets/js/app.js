...
//Hover при наведении на картинку Rank
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function animateCounter(counterElement) {
    const finalCountString = counterElement.getAttribute("data-value");

    const finalCount = parseFloat(finalCountString);

    const hasDecimalPart = finalCountString.includes(".");

    const animationDuration = 5000; // 5 секунд

    const animationSteps = 150; // Уменьшаем количество шагов

    const stepDuration = animationDuration / animationSteps;

    const stepValue = finalCount / animationSteps;

    let currentCount = 0;

    function updateCounter() {
      const formattedCount = hasDecimalPart
    ...
    function step() {
      ...
      if (currentCount < finalCount) {
        ...
        // Чтобы убедиться, что финальное значение точно отображается
